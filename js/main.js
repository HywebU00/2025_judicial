...
function mainMenuSetup() {
  ...
  hasChild.forEach((item) => {
    const triggerA = item.querySelector(':scope > a');
    const childUl = item.querySelector(':scope > ul');

    if (!triggerA || !childUl) return;
    ...
    triggerA.addEventListener('click', (e) => {
      ...
      const isExpanded = triggerA.getAttribute('aria-expanded') === 'true';

      // Toggle 自己
      triggerA.setAttribute('aria-expanded', isExpanded ? 'false' : 'true');
      childUl.setAttribute('aria-hidden', isExpanded ? 'true' : 'false');
      ...
      siblings.forEach((sib) => {
        ...
        const sibUl = sib.querySelector(':scope > ul');
        if (sibA && sibUl) {
          sibA.setAttribute('aria-expanded', 'false');
          sibUl.setAttribute('aria-hidden', 'true');
